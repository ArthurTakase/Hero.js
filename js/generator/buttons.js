...
function addOption() {
    ...
    switch (optionCondition.value) {
        ...
        case "SKILL":
            json.condition = optionCondition.value
            try {json.conditionData = [conditionIsHere.checked, skillsList[conditionData.value].name]}
            catch(e) {return}
            break
        case "OBJECT":
            json.condition = optionCondition.value
            try {json.conditionData = [conditionIsHere.checked, objectsJson[conditionData.value].type, objectsJson[conditionData.value].name]}
            catch(e) {return}
            break
        default: break
    }
    ...
    switch (optionEffect.value) {
        case "REMOVE_OBJECT": case "ADD_OBJECT":
            ...
            try {json.effectData = [objectsJson[effectData.value].type, objectsJson[effectData.value].name]}
            catch(e) {return}
            break
    ...
    else {buttonList.push(json); optionIDGlobal += 1}

    // ===== TABLEAU =====
    liste.innerHTML = '<tr class="tableHeader"><th>ID</th><th>Text</th><th>Go To</th></tr>'
    for (button in buttonList) {
        if (buttonList[button].goToIndex == undefined) {liste.innerHTML += '<tr><td>' + button + '</td><td>' + buttonList[button].text + '</td><td>Next</td></tr>'}
        else {liste.innerHTML += '<tr><td>' + button + '</td><td>' + buttonList[button].text + '</td><td>' + buttonList[button].goToIndex + '</td></tr>'}
    }
